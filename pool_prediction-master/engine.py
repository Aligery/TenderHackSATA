...
dsn_database = "count"         # e.g. "MySQLdbtest"
...
threshold = 0.05
while True:
    btc = open('live_count.csv','r')
    sent = open('live_tweet.csv','r')
    cou_data = btc.readlines()
    sent_data = sent.readlines()
    cou_data = process_data(cou_data[len(cou_data)-5:])
    sent_data = process_data(sent_data[len(sent_data)-5:])
    live = scaler.transform(cou_data)
    testX, testY = create_dataset(live, 2, sent_data, sent=True)
    testX = np.reshape(testX, (testX.shape[0], 1, testX.shape[1]))
    yhat = model.predict(testX)
    yhat_inverse = scaler.inverse_transform(yhat.reshape(-1, 1))
    true_q.put(cou_data[4])
    pred_q.put(yhat_inverse[0])
    val = 100*((yhat_inverse[0][0] - prev)/prev)
    if val > threshold:
        decision = 'close'
        message = client.messages.create(to="+15184234418‬", from_="+15188883052", body=decision+' - Price of this count is expected to rise.')
    elif val <-threshold:
        decision = 'red'
        message = client.messages.create(to="+15184234418", from_="+15188883052", body=decision+' - Price of this count is expected to drop.')
    else:
        decision = ''
    prev = yhat_inverse[0][0]
    input_string = "INSERT INTO live_data values ({},{},{},'{}','{}');".format(yhat_inverse[0][0],cou_data[0][0],sent_data[4][0],datetime.datetime.now(tz=est).strftime('%Y-%m-%d %H:%M:%S'),decision)
